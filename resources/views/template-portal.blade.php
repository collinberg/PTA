        ...
        <div class='flex flex-wrap'>
          <main class='w-full lg:w-3/4 lg:pr-10'>
            ...
            <div class='file-grid mb-10'>
              @foreach($document_repeater as $document)
                <!-- File -->
                <div class='file-row flex flex-row border border-grey mb-2 content-center'>
                  ...
                  <div class='flex-1 p-3'>
                    <p class='text-primary font-bold mb-1'>{{ $document['document_title'] }}</p>
                    <p>{{ $document['document_description'] }}</p>
                  </div>
                  <div class='bg-grey text-center hover:bg-grey-dark transition duration-500'>
                    <a class='text-sm block py-5 px-8 font-bold border-none text-black' download href="{{ $document['document_file'] }}"><i class='fas fa-download text-primary text-2xl mb-1'></i><br> Download</a>
                  </div>
                </div>
              @endforeach
            </div>
